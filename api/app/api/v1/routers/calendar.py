...
def create_scheduled_content_item(
    ...
    return service.create_item(user_id=user.user_id, payload=payload)
...
def list_calendar_items(
    ...
    return service.list_items(user_id=user.user_id, limit=limit)
...
def update_item_status(
    ...
    try:
        return service.update_status(item_id=item_id, user_id=user.user_id, status=payload.status)
...
def move_item_date(
    ...
    try:
        return service.move_item_date(
            item_id=item_id, user_id=user.user_id, scheduled_for=payload.scheduled_for
        )
    except LookupError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
def delete_item(
    ...
    try:
        result = service.delete_item(item_id=item_id, user_id=user.user_id)
